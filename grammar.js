...
module.exports = grammar({
  ...
  rules: {
    ...
    class_service_modifier: $ => seq(
      $.service_category_modifier,
      optional(seq('(', $.projection_reference, ')')),
    ),
    ...
    enumeration_literal: $ => seq(
      $.identifier,
      optional(seq('(', $.enumeration_pretty_name, ')')),
      ',',
    ),
    ...
    association_block: $ => seq(
      '{',
      optional($.association_end),
      optional($.association_end),
    ...
    projection_primitive_member: $ => seq(
    ...
    projection_reference_property: $ => seq(
      optional(seq($.classifier_reference, '.')),
      $.identifier,
      ':',
      $.projection_block,
    ...
    projection_projection_reference: $ => seq(
      optional(seq($.classifier_reference, '.')),
      $.identifier,
      ':',
    ...
    projection_parameterized_property: $ => seq(
      optional(seq($.classifier_reference, '.')),
      $.identifier,
      $.argument_list,
      ':',
    ...
    service_criteria_declaration: $ => seq(
      $.service_criteria_keyword,
      ':',
      $.criteria_expression,
      ';',
    ),
    service_criteria_keyword: $ => choice('criteria', 'authorize', 'validate', 'conflict'),
    service_projection_dispatch: $ => seq(
      'projection',
      ':',
      $.projection_reference,
    ...
    parameterized_property_signature: $ => seq(
      $.identifier,
      '(',
      optional(sep1($.parameter_declaration, ',')),
      ')',
    ...
    // ---- validations ----
    data_type_property_validation: $ => choice(
    ...
    primitive_parameter_declaration: $ => seq(
      $.identifier,
      ':',
      $.primitive_type,
      $.multiplicity,
      repeat($.parameter_modifier),
    ),
    enumeration_parameter_declaration: $ => seq(
      $.identifier,
      ':',
      $.enumeration_reference,
      $.multiplicity,
    ...
    // ---- order by ----
    order_by_declaration: $ => seq(
      'orderBy',
      ':',
      sep1($.order_by_member_reference_path, ','),
    ),
    order_by_member_reference_path: $ => seq(
      $.this_member_reference_path,
      optional($.order_by_direction),
    ),
    order_by_direction: $ => choice('ascending', 'descending'),

    // ---- criteria ----
    // ANTLR criteriaExpression is left recursive with && (higher) then ||.
    criteria_expression: $ => choice(
    ...
    criteria_expression_and: $ => prec.left(2, seq(
      field('left', $.criteria_expression),
      '&&',
      field('right', $.criteria_expression),
    )),
    criteria_expression_or: $ => prec.left(1, seq(
      field('left', $.criteria_expression),
      '||',
      field('right', $.criteria_expression),
    )),
    ...
    criteria_all: $ => 'all',
    criteria_operator: $ => seq(
      field('source', $.expression_value),
      $.operator,
      field('target', $.expression_value),
    ),
    ...
    expression_member_reference: $ => choice(
      $.this_member_reference_path,
      $.type_member_reference_path,
    ),
    ...
    this_member_reference_path: $ => seq(
      'this',
      repeat(seq('.', $.association_end_reference)),
      '.',
      $.member_reference,
    ),
    type_member_reference_path: $ => seq(
      $.class_reference,
      repeat(seq('.', $.association_end_reference)),
      '.',
      $.member_reference,
    ...
    keyword_valid_as_identifier: $ => choice(
      'package',
    ...
    integer_literal_token: $ => token(choice(
      /0[xX][0-9a-fA-F_]+[lL]?/,
      /0[bB][01_]+[lL]?/,
      /0[0-7_]+[lL]?/,
      /(0|[1-9][0-9_]*)[lL]?/,
    )),
    floating_point_literal_token: $ => token(choice(
    ...
    character_literal_token: $ => token(seq(
      "'",
      choice(/[^'\\\r\n]/, /\\['btnfr"\\]/, /\\[0-3]?[0-7]?[0-7]/, /\\u+[0-9a-fA-F]{4}/),
      "'",
    )),
    string_literal_token: $ => token(seq(
      '"',
      repeat(choice(/[^"\\\r\n]/, /\\['btnfr"\\]/, /\\[0-3]?[0-7]?[0-7]/, /\\u+[0-9a-fA-F]{4}/)),
      '"',
    )),
