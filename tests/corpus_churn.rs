...
fn idempotency() {
    ...
    assert!(failures.is_empty(), "idempotency failures:\n{}", failures.join("\n"));
}
...
fn churn() {
    ...
    for path in &files {
        ...
        match klassfmt::format(&src) {
            ...
            Err(e) => {
                worst.push((usize::MAX, format!("{} (ERROR: {e})", path.file_name().unwrap().to_string_lossy())));
            }
        }
    }
...
fn no_churn() {
    ...
    assert!(failures.is_empty(), "{} files still churn:\n{}", failures.len(), failures.join("\n"));
}
