...
//!
//! Each listed file is a canonical fixed point: formatting it must reproduce it
//! byte-for-byte. These lock in the formatting of a spread of constructs
//! (classes, associations, projections with nested blocks, inheritance,
//! parameterized properties, criteria) so regressions surface immediately.
...
fn assert_fixed_point(name: &str) {
    ...
    assert_eq!(
        formatted, src,
        "{name} should format to itself unchanged (golden fixed point)"
    );
}
